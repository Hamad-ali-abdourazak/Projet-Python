...
import xmltodict
import datetime
from Classes import Document, Author, NewsDocument, ArxivDocument
...
def fetch_newsapi_articles(api_key, query, limit=100):
    try:
        ...
        for i, article in enumerate(data['articles']):
            ...
            source = article['source']['name'] if 'source' in article else "Source Inconnue"
            articles.append((titre, contenu, auteur, date, source))
...
for i, (titre, contenu, auteur, date, *source) in enumerate(docs):
    if len(contenu) < 100:
        docs.remove((titre, contenu, auteur, date, *source))

# Création d'une chaîne de texte avec tous les documents restants
longueChaineDeCaracteres = " ".join([contenu for titre, contenu, auteur, date, *source in docs])

# Construction des objets Document à partir des données NewsAPI et ArXiv
collection = []
for doc in docs:
    if len(doc) == 5:  # NewsDocument
        titre, contenu, auteur, date, source = doc
        date_obj = datetime.datetime.strptime(date, '%Y-%m-%dT%H:%M:%SZ')
        date_str = date_obj.strftime('%Y/%m/%d')
        document = NewsDocument(titre=titre, auteur=auteur, date=date_str, url="", texte=contenu, source=source)
    else:  # ArxivDocument
        titre, contenu, auteurs, date = doc
        date_obj = datetime.datetime.strptime(date, '%Y-%m-%dT%H:%M:%SZ')
        date_str = date_obj.strftime('%Y/%m/%d')
        document = ArxivDocument(titre=titre, auteur=auteurs[0], date=date_str, url="", texte=contenu, co_auteurs=auteurs[1:])
...
print(corpus.search("climate"))
print(corpus.concorde("climate", context_size=30))
corpus.stats(n=10)